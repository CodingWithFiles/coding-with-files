...
- Run `.cig/scripts/command-helpers/context-manager hierarchy <parent-path>` using the Bash tool to resolve parent task directory and verify it exists.
- Run `.cig/scripts/command-helpers/context-manager inheritance <parent-path>` using the Bash tool to load parent context (structural map for scope constraints).

...
allowed-tools: Read, Bash(.cig/scripts/command-helpers/hierarchy-resolver.pl:*), Bash(.cig/scripts/command-helpers/format-detector.pl:*), Bash(awk:*), Bash(egrep:*), Bash(echo:*)
---
...
- Otherwise: Treat as task-path and resolve using hierarchy-resolver.pl

### 2. Resolve Task and File (for task-based paths)
- Use `hierarchy-resolver.pl <task-path>` to find task directory
...
- Use `format-detector.pl <task-dir> <workflow-file>` to determine which file to read

...
## Task 232: sync docs and README with current CWF state

### Status: Complete (2026-07-18)
### Duration: chore phases a,d,e,f,g,j; plans in a prior session, exec+retrospective this session; ~1-day estimate, on/under target (audit front-loaded the thinking, exec single-pass).
### Impact: The recurring headline-doc sync (the Task 189 precedent, ~42 tasks of drift later at v1.1.231) — bring README and the maintainer/spec docs back into agreement with what ships, as the other half of preparing the repo for public release (Task 231 scrubbed private data). **Drift was smaller than Task 189**: an audit-first scope (2 Explore agents + 5 plan reviewers) found only **two substantive fixes**, the rest cosmetic. Four docs edited (CLAUDE.md, DESIGN.md, README.md, CWF-PROJECT-SPEC.md); **COMMANDS.md and INSTALL.md audited clean and left untouched** (recorded as audited, not skipped). **The load-bearing fix corrected an *inverted* invariant, not merely a stale one**: CLAUDE.md/DESIGN.md described the security model as "u+rx (minimum 0500)" — a permissions **floor** — when the real Task-170 model is a **ceiling** (recorded `permissions` are an upper bound; `cwf-manage validate` flags a file only when *more* permissive than recorded; `fix-security` clamps down `actual & recorded`, never raising; recorded modes span `0700`/`0500`→`0444`). The reworded prose was verified against `hash-updates.md` L20 and the actual `cwf-manage` clamp logic **before** commit and independently re-confirmed by the robustness/security/misalignment reviewers. **Second substantive fix**: two installed skills (`cwf-current-task`, `cwf-backlog-manager`) had never been added to CLAUDE.md's list — added in the existing Core/Utility grouping, keeping `cwf-manage` labelled the management **script**. **Version-example hygiene split by class** (plan-review policy): `git describe` **format** examples (`v1.1.187-5-gcea1c19`) were genericised to their shape (`<tag>-<commits-since>-g<short-sha>`, e.g. `v1.1.x-<n>-g<sha>`) so they never re-stale, while the **config-value** samples (schema `/^v\d+\.\d+\.\d+$/`, so ungenericisable) were an **owner decision** — refreshed to **v1.1.232** (the version this very task bumps `last_released` to and the tag before the public push, so the samples are forward-accurate, not re-stale). README's three "always split planning/execution" overstatements were hedged to COMMANDS.md's "where applicable" form (only implementation and testing have `-exec` phases); the file-structure line was deliberately **left** (robustness review — avoid over-correction). **The a-plan's "23 skills" was a miscount** (it counted 2 loose `.md` fragments in `.claude/skills/` as commands); the audit agents caught it, it was reconciled to 21 dirs / 20 commands at d, and a Step-1 divergence rule was added so exec recounts rather than trusts the number (recount matched at exec — no divergence). **The generated-artefact smoke test from the a-plan was removed at plan review** — prose edits do not flow into template output (not a rebrand), so the grep sweep is the complete check; `scratchpad.md` is untracked → out of scope. **Docs-only, zero integrity friction**: none of the 4 edited paths is in `script-hashes.json` (verified), so no hash refresh; `cwf-manage validate` OK; suite `Files=78, Tests=1077, PASS`. All 7 changeset reviews (5 at f, 2 at g) `no findings`; TC-1..TC-9 all PASS. **No backlog item completed** (a docs sync ships no guard) and **none added** — the automation candidates it surfaced are already tracked: the recurring manual skill-list fix is exactly what backlog item **"README skill-list drift guard (documented vs shipped /cwf-* set)"** would automate, the broader case is covered by **"Doc-drift check when a task changes an enforced shape"**, and the residual "CWF"-in-prose usage belongs to **"Branding cleanup: CWF to CwF rebrand…"** (correctly out of scope — this sync introduced no new "CWF"/"CwF" prose token). Merge to main is human-only and outstanding.

## Task 231: scrub private data from docs and history

### Status: Complete (2026-07-18)
### Duration: bugfix phases a,c,d,e,f,g,j across ~3 calendar days; ~1-day estimate, +200% (all follow-on discovery, not slippage on the planned redaction).
### Impact: Built and verified the tooling to remove the maintainer's private data — username-bearing absolute paths (slash **and** dashified `home-<user>` forms), ~10 other-project names, and the personal non-public `@mattkeenan.net` address (keeping the already-public `github@mattkeenan.net`) — from **every** commit's file content, commit-message bodies, and the 30 annotated-tag messages, before the repo is made public. **The task ships no redaction diff**: a history rewrite cannot be a branch diff, so this branch commits only its own workflow docs; the redaction ruleset (`redact-rules.txt`) and drivers (`scrub.sh`, `verify.sh`) live **scratch-only, never committed** (D4 — they *are* the private data by construction), and the live rewrite + push is staged as a **human-only runbook** (f-exec Step 4), the deliberate task boundary. **Mechanism**: one `git filter-repo --replace-text --replace-message` pass from a single rules file drives content *and* message redaction across all refs; because filter-repo migrates every source branch into one consistent object map, the `git rev-list --all` verification pass genuinely covers **all 432 refs** (235 task + 197 checkpoint branches), not just `main` — proven empirically (433 remote-tracking → 432 local heads, 0 leak hits). **Scope widened by owner decision** three times: from `implementation-guide/**` to the whole tracked tree + message bodies; **`lmm` retained entirely** (entangled with the `mcp__lmm__*` API namespace and a tracked directory path that `--replace-text` cannot rewrite — scrubbing content-only would be inconsistent for no gain); and the task's own a/c/d/f/g workflow docs **genericised to categories** so publishing the task record does not re-seed the very data it removes ("no point redacting the repo only to add it back in the task docs"). **Two real leak classes were caught pre-exposure, both by downstream checks rather than inventory**: the dashified-username path form (`home-<user>` with a hyphen, invisible to slash-anchored rules — 72 committed docs; caught by the changeset review) and a self-leak in f-exec where a made-up Class-B example string matched the leak pattern and was rule-uncovered (G1, caught by the TC-4 all-history scan) — fixed by *describing* plant strings rather than embedding them, now a recorded rule. **Verification is negative-control-first**: `verify.sh` was proven to *fire* on a planted Class-A/Class-B leak and *not* fire on the kept address before any PASS was trusted; TC-1..TC-12 all green on a disposable `--no-local` clone (never the live repo — the "test DB, never production" rule for history). Integrity: post-scrub `validate` shows **zero sha256/content/existence violations** — the only drift is umask-derived permission drift (git records only the exec bit, so recorded 0500/0444 always reset on a fresh checkout), cleared by a `fix-security` step now baked into the runbook; suite `Files=78, Tests=1077`. **Exposure reality, surfaced not smoothed**: `origin/main` already carries 58 private-data files, so the scrub is *remediation* of a live exposure, not only prevention; with **0 PRs and 0 forks** confirmed, a `main`-only force-push suffices (GC reclaims the unreferenced old commits — no Support purge needed), but **tag re-push is deferred** (owner) — the 22 public tags still point at old unscrubbed commits and a force-push does not move them, leaving that exposure open until they are separately force-updated or deleted (captured as future work). Runbook push is scoped to `main` only — **never `--all`/`--tags`/`--mirror`**, which would publish the 432 local branches and 17 local-only tags that are deliberately kept private. One CWF tooling defect logged to BACKLOG: `security-review-changeset --wf-step=testing-exec` under-covers a tracked-file change on phase re-execution (same family as Task 141). Merge to main is human-only and outstanding; the live rewrite/push runbook is likewise human-only.

## Task 230: cwf-new-subtask omits git branch creation

### Status: Complete (2026-07-14)
### Duration: bugfix phases a,c,d,e,f,g,j in one session; <1 day estimate, on target.
### Impact: Fixed a long-standing omission in `.claude/skills/cwf-new-subtask/SKILL.md`: it never created a git branch, so subtask work silently accumulated on the parent task's branch. **The bug was original, not a CwF-upgrade regression** — git archaeology (pickaxe `git log -S` + `git blame` across the `cig-subtask`→`cwf-subtask`→`cwf-new-subtask` rename chain) traced the missing branch step to the skill's very first version (`.claude/commands/cig-subtask.md`, `604201d`, 2025-08-23), with the false "There is **no `git checkout -b`** in this skill — the subtask stays on the parent branch" prose added later by Task 203 (`186d539`, blessing the omission as intentional). **The whole workflow had silently compensated**: the retrospective's Suggest Merge (`retrospective-extras.md:147,152-155`) computes a subtask branch `<type>/<num>-<slug>` and ff-merges it into the parent branch — a branch that was never created — and `CWF::TaskContextInference` was already written to resolve decimal subtask branches. Because those consumers dictated the branch name and base, the fix reduced to **mirroring `cwf-new-task` step 4**: insert `### 4. Create Git Branch` (`git checkout -b "<type>/<num>-<slug>"`, byte-identical command), renumber Provision-Scratch 4→5 and Provide-Next-Steps 5→6, delete the false prose (pure deletion — the mirror source carries no ordering sentence), and surface the branch in Next Steps and Success Criteria. **One file changed; `.claude/skills/` is not hash-tracked** (verified — zero `cwf-new-subtask`/`claude/skills` entries in `script-hashes.json`), so no `script-hashes.json` refresh. **Step 4 is a documented, deliberate divergence** from the bare top-level step, not a verbatim mirror: it pins three subtask-specific facts — decimal `<num>`, **reuse the script-produced slug (do not re-derive it)** as the FR4(e) sanitisation invariant, and the ff-merge-into-parent precondition — reconciling the security reviewer's slug-injection concern with the improvements reviewer's "own the divergence, don't over-claim exact mirror" framing. **The skill session-cache shaped testing**: an edited `SKILL.md` is not live in the same session (skills load at session start), so TC-4/5/6 executed the *documented procedure by hand* (`task-workflow create` a throwaway `230.1` subtask, `git checkout -b`, assert HEAD-is-subtask-branch + parent-is-ancestor + branch-signal-correlates + retrospective ff round-trip, then delete the throwaway dir + branch), with the live `/cwf-new-subtask` invocation (TC-8) deferred to a fresh session. TC-5 confirmed the misalignment reviewer's catch — the branch signal now resolves the subtask's *own* decimal (`task_num: 230.1`, correlated), a benign behaviour change from Task 166's "subtasks share the parent branch" premise requiring no code change. Full suite `Files=78, Tests=1077, PASS`; all changeset reviews clean (5 at f, 2 at g). One follow-up seeded: audit `cwf-new-task` step 4 for the same unpinned-slug ambiguity. Merge to main is human-only and outstanding.

## Task 229: unify sandbox non sandbox scratch path

### Status: Complete (2026-07-13)
### Duration: feature phases a–j in one session; 1–2 day estimate, in band (effort front-loaded into design).
### Impact: Made CwF's per-task scratch path resolve to one canonical location a path-based permission rule can match whether the session is sandboxed or has fallen back to non-sandbox. Root cause was that `scratch_parent` derived the base from `$TMPDIR`, which differs by process context — **unset** in the unsandboxed context-inject hook, `/tmp/claude-<uid>` in the sandboxed Bash tool, and different again on off-sandbox fallback — so the path the hook *advertised* could differ from the path a writer *actually wrote*, and an allowlist keyed on one form silently missed the other; a real user report also showed non-idempotent **doubling** (`cwf-<slug>/cwf-<slug>/…`) when the harness set `$TMPDIR` to a value that already contained the scratch parent. **The headline is the mechanism: the fix does not reconcile `$TMPDIR` across contexts, it stops reading `$TMPDIR` entirely** — the base is now a package scalar `$SCRATCH_BASE = "/tmp/claude-$>"` derived from the EUID alone (`$>`), the only mode-invariant, non-attacker-influenceable input that names the sandbox's writable per-uid temp on Linux/WSL2. Because `$TMPDIR` never enters the path, the doubling is structurally impossible and the env-var-injection class (`..`, relative, hostile values) is gone — a net **removal** of code and attack surface: `scratch_parent` is now pure string work returning `"$SCRATCH_BASE/cwf$dashed"`, the whole env→probe→`/tmp` base-selection branch and `$SANDBOX_TMP_PROBE` are deleted, and the second derivation point (`best-practice-resolve::scratch_out_path`, which reimplemented the `$TMPDIR` derivation and would have missed the fix) now delegates to `scratch_dir`. The new predictable `/tmp/claude-<euid>` directory in world-writable `/tmp` is covered by extending the existing race-tolerant guard to **two levels** (`mkdir -m 0700` then `-l` reject, base before parent, ordered mkdir-then-check to avoid a TOCTOU regression); the containment boundary stays the atomic `0700` create + fail-closed write, the `-l` check is defence-in-depth. A new exported `scratch_fail_hint($kind)` gives all three `scratch_dir` callers a uniform, cause-naming diagnostic on `mkdir_failed`/`symlink_parent` — **which superseded design decision D3**, whose proposed return-shape extension (attempted path in slot 1) was refined during implementation planning into this separate helper, keeping `scratch_dir`'s `(undef, $kind)` contract intact (c-design left as the historical record per the per-phase-checkpoint model). **One accepted regression, surfaced not smoothed**: hardcoding `/tmp/claude-<euid>` fails **closed** on a macOS Seatbelt sandbox (whose writable temp is under `/var/folders`), where today's `$TMPDIR`-reading code works — a deliberate owner decision to keep the Linux design pure now and restore macOS via the Medium backlog item **"Platform-specific scratch base (Linux/macOS/…)"** (added this task); the failure is loud and names the base as the cause, never silent. Four hash-tracked files refreshed in the exec commit (`CWF::Common.pm`, `best-practice-resolve`, `security-review-changeset`, `plan-mechanical-check`); `tmp-paths.md` rewritten to the EUID base with the macOS limitation and the two-level guard; `cwf-manage validate` OK. `t/scratch.t` reworked to 13 cases (seam swept from `local $ENV{TMPDIR}` to `local $CWF::Common::SCRATCH_BASE`, oracle re-pointed at hard-coded literals to avoid mirroring the implementation) with two new standing regression guards — **TC-10** poison-`$TMPDIR` invariance (the reporter's bug) and **TC-11** intermediate-symlink reject; two `security-review-changeset.t` cases needed unplanned changes because they encoded the *old* `$TMPDIR`-honouring behaviour — TC-TMPDIR-1/2/3 flipped from "honours `$TMPDIR`" to "invariant under `$TMPDIR`", and **TC-209-2** (char-device-doesn't-abort) now **skips on the exact `scratch unavailable (mkdir_failed)` signal** because `unshare -rm` remaps the process to uid 0 → `/tmp/claude-0` (uncreatable under a read-only `/tmp`), a fail-closed error distinct from the Task-209 abort it guards; the owner chose skip-on-signal over building bind-mount/tmpfs scaffolding to synthesise a writable uid-0 base, so a genuine abort still fails loudly and the case runs normally anywhere the base is creatable. Full suite `Files=78, Tests=1077, PASS`; the g-phase smoke test confirmed hook/writer path parity (`best-practice-resolve`'s `.out` landed byte-identical to the `CWF PATHS` hook-advertised parent). All changeset reviews clean (5 at f, 2 at g) bar one low-severity best-practice finding (`t/scratch.t sub dash` reading `$_[0]` instead of unpacking `@_`), fixed in-phase. Rollout is an in-place library update shipped via `cwf-manage update` with **no phased rollout** (no runtime service, no user cohort); rollback is `cwf-manage rollback`/revert with no migration since scratch is ephemeral. Merge to main is human-only and outstanding.

### Retired Backlog Items
#### Extract a shared CWF::Common::tmp_base() helper
...
<!-- Note: Superseded by Task 229: the $TMPDIR base-selection and $SANDBOX_TMP_PROBE it proposed to deduplicate were removed outright. The scratch base is now the single EUID-derived $SCRATCH_BASE scalar, and the second derivation point (best-practice-resolve) delegates to scratch_dir. The dedup goal is met by elimination, so no tmp_base() helper is needed. -->

## Task 228: R4 unresolved-decisions gate for a-task-plan and mechanism-named-AC ban

### Status: Complete (2026-07-12)
### Duration: feature phases a–j in one session; ~1-day estimate, on target (calendar estimate treated as noise per Task 219 S7).
### Impact: Delivered backlog item **R4** (Task 219, Med) — the last of Task 219's cross-project synthesis findings picked up here — which S5/R4 traced across 6 projects (gocryptoknock, lensman, thenetworking, gresearch, gate, lmm, terminfo): late surface/mechanism/constraint decisions (chat transport, firewall model, UUID anchor, project layout) routinely surfaced at design or exec instead of task-plan, driving ~3× re-plans, and acceptance criteria named after a chosen mechanism aged badly after a pivot so "done" tracked the mechanism instead of the outcome. Two deliverables, **guidance-only**, on the single `a-task-plan` surface: (1) an **open-decisions gate** — the plan author is now prompted to name every still-open surface/mechanism/constraint choice as a question to resolve in requirements/design; (2) a **mechanism-named-AC ban** — success criteria are steered toward observable outcomes and away from not-yet-chosen mechanisms. **The load-bearing decision was enforcement altitude (design D1): guidance, not a deterministic check** — because `plan-mechanical-check` never scans `a-task-plan`, the a-phase has no review MAP to host a check, and "mechanism-named" is too fuzzy to flag without the false positives **NFR5** forbids; "guidance vs check" is an altitude decision, not a strength one, and the fuzzy-rule case is exactly where a low-false-positive mechanical check may not exist. Three surfaces edited (design D2), **all three untracked by `script-hashes.json`** (verified in the f-phase smoke check) → zero hash friction, no `script-hashes.json` refresh: `.cwf/docs/workflow/workflow-steps/planning.md` is the **authority** (the mechanism-named definition + litmus test + four worked examples — `✗ "Ranks results with a Redis sorted-set." / ✓ "Returns results in rank order."`, `✗ "Adds a --json CLI flag." / ✓ "A caller can obtain the report as machine-readable output."` — plus a new Focus-on bullet, two Key Questions, and **both** Avoid tension lines reconciled with the naming≠choosing carve-out); `.cwf/templates/pool/a-task-plan.md.template` is the **prompt** (a new `## Open Decisions` H2 between `## Constraints` and `## Decomposition Check`, with the `None open — <justification>` escape hatch marking a bare "None" non-conformant, plus the outcome-shaped-criteria note under `## Success Criteria`); `.claude/skills/cwf-task-plan/SKILL.md` is the **gate** (two `## Success Criteria` checklist items mapping 1:1 to FR1/FR2 — D1's sole enforcement surface). **D3 kept `## Open Decisions` a distinct new H2, not folded into `Constraints`; D5 made the change additive-only** — structural readers key off `**Status**:` markers and named headers, not positional order, so **TC-3 proved the pre/post template parses byte-identically** (the added mid-document H2 is invisible to `status-aggregator-v2.1` / `workflow-manager status --workflow` by construction) — additivity proven by regression, not asserted. **The feature dogfooded itself during its own planning**: `a-task-plan.md` carried an `### Open Decisions` subsection (gate location, enforcement altitude, definition, R6/R13 boundary) and kept its own ACs outcome-shaped before the template was even edited — and those four named decisions mapped almost 1:1 onto design decisions D1–D5, making design resolution rather than discovery (the exact benefit R4 exists to produce). **One in-phase fix** (misalignment reviewer, f-phase): two new bare `planning.md` cross-references (template Success-Criteria comment; skill checklist item) diverged from the intra-repo backtick convention (`docs/conventions/cross-doc-references.md`: `inline-backtick × path`) — both backticked before the checkpoint, not deferred. No code shipped (D1) so no line/branch coverage; TC-1..TC-6 all PASS (TC-1 used a throwaway **chore**-type task, confirming the pool edit reaches non-feature types via the per-type symlinks). `cwf-manage validate` OK throughout; all 7 changeset reviews (5 at f, 2 at g) clean bar the one fixed misalignment finding. Rollout is squash-to-main + human tag → downstream `cwf-manage update` (additive, trivially revertible single commit); merge to main is human-only and outstanding. **Scoped clear of R6** (complexity tier + risk register) and **R13** (extend `plan-mechanical-check`), which also touch `a-task-plan`; whoever picks them up should reuse the `## Open Decisions` H2 placement precedent so the three additions do not collide. No new backlog item created; the Task 219 umbrella item retains its remaining follow-ups.

## Task 227: R3 shell-hygiene convention and Bash allowlist seed

### Status: Complete (2026-07-12)
### Duration: feature phases a–j in one session; Medium complexity, on target (calendar estimate treated as noise per Task 219 S7).
### Impact: Delivered the two remaining halves of backlog item **R3** (Task 219, Med) — the *documentation* and *friction-reduction* halves, distinct from the *enforcement* half Task 220 already shipped and the path-injection-hook residual Task 224 closed. With this, R3 is fully delivered across Tasks 220+224+227. Two deliverables: (1) a shipped `.cwf/docs/conventions/shell-hygiene.md` convention doc — prompt-free/portable shell idioms (heredoc/inline-script avoidance → scratch, `chmod +x && ./script`, no `perl -c`/`bash -n`, prompt-tripping substitution avoidance, NUL-separated git paths), *linking* rather than restating the anti-pattern table and tool-tier rubric, plus the allowlist-seed section (admission criterion, excluded-near-neighbour table, deny/ask opt-out, harness-matching caveat) — wired to runtime via the load-bearing FR3 anchor in `cwf-agent-shared-rules.md` (read by shell-doing subagents) and this repo's `CLAUDE.md`; (2) a read-only Bash allowlist seed — a 5-entry compile-time `@READ_ONLY_ALLOWLIST` constant in `cwf-claude-settings-merge` (`ls`, `pwd`, `git status`, `git rev-parse` as `:*` prefixes + `git branch --show-current` **pinned exact**) appended to the manifest-derived entries and riding the **single existing** `merge_allow` call — additive, idempotent, no second writer. **The load-bearing safety decision was exact-vs-prefix**: `git branch --show-current` is pinned exact precisely because the `git branch:*` prefix would admit `git branch -D` — a read-only allow rule must be read-only for its *whole* glob space or pinned exact. **The test gate is deliberately anti-tautological**: `is_read_only_safe($entry)` validates the shipped corpus against two independent hand-authored membership sets (`%SAFE_PREFIX_KEYS`/`%SAFE_EXACT_KEYS`) authored from the admission criterion, *not* from the script corpus — so the gate can catch a bad future corpus edit instead of rubber-stamping it. The predicate uses `substr`-based `:*` suffix splitting (no backtracking regex) + `\A`/`\z` + `/aa` anchoring; TC-RO1..RO5 cover accept/reject controls (incl. the prefix-form-of-exact near-miss and a trailing-newline anchor probe), exact/prefix split, clean-merge corpus presence, additive, and idempotent. **Reuse over new machinery** (Improvements reviewer: no findings): the seed rides `merge_allow`/`partition_manifest`, mirrors the `$CANONICAL_*` constant pattern, and the doc links rather than restates. **One in-phase fix**: the misalignment reviewer caught `shell-hygiene.md` using markdown-link cross-refs where `cross-doc-references.md` prescribes `inline-backtick × path` — converted the four references, not deferred. **One deviation caught by `validate`, not the plan**: the plan named only the helper for hash refresh, but editing `cwf-agent-shared-rules.md` also required one — both sha256 entries refreshed in-task and independently verified with `sha256sum`. **Surface, never smooth**: the undocumented question of whether a `Bash(<verb>:*)` allow rule auto-approves redirection (`>`,`>>`) or command substitution (`$(…)`,backtick) could not be probed authoritatively offline (needs a controlled single-rule harness fixture); rather than assume-safe or block, the doc status was verified (undocumented, harness-wide — affects every `allow` entry, not just this corpus), a hard caveat written into `shell-hygiene.md`, and a Medium discovery backlog item seeded. `cwf-manage validate` OK; full suite `Files=78, Tests=1078, PASS`; all 7 changeset reviews (5 at f, 2 at g) clean bar the one fixed misalignment finding. Rollout is trunk-merge + downstream `cwf-manage update` (additive+idempotent → rollback is a revert-merge); merge to main is human-only and outstanding.

## Task 226: separate goals from requirements in plan stage

### Status: Complete (2026-07-12)
### Duration: bugfix phases a,c,d,e,f,g,j in one session; ~0.5 day estimate, on target.
### Impact: Closed a plan-stage instruction defect where requirements-phase discipline leaked into the goals phase. Trigger was a real CwF-user incident (referenced as the Task-31 incident): the goals-step reference (`planning.md`) carried both the "best part is no part" simplicity maxim **and** a lossy "capture the single-sentence *why*" instruction, so an agent paraphrased a user's concrete request into a looser intent and then applied requirement-cutting logic to a **goal** — a review agent nearly deferred a deliverable the user had explicitly named. The fix draws the goal/requirement boundary the phase structure (`cwf-task-plan`/a vs `cwf-requirements-plan`/b) already implies, encoding it as instruction text rather than a prose caveat so it is agent-enforceable. Six design decisions across six files, zero scope change: **KD1** dual-capture — the goals step now records **both** the "why" and the user's explicit request (named deliverables verbatim, "none stated" allowed for genuine one-liners), replacing the lossy single-sentence objective in `planning.md`, `cwf-task-plan/SKILL.md`, and the `a-task-plan.md.template` pool source; **KD2** relocation-not-weakening — "best part is no part" stays **universal** in `planning.md` but is **fenced to the means** (never the goal or named deliverables), with the fuller challenge-every-requirement discipline added to `requirements.md`; the plan named over-correction (blunting the maxim) as its top risk and TC-2 asserted the maxim survives in *both* docs (the no-orphan check), so bugfix/hotfix/chore — which lack a `b` phase — still see it inline; **KD3** goals are owner-owned/near-inviolable, no unilateral narrow-or-expand; **KD4** the scope-surfacing obligation binds the plan/exec **review agents** too, landed in **one** shared file (`cwf-agent-shared-rules.md`, hash-tracked) rather than 10 reviewer defs — the misalignment reviewer independently confirmed all 10 defs still link it and none were edited; **KD5** baked the rule into the SKILL Success-Criteria checklist (statically assertable) not prose alone; **KD6** same-commit sha256 refresh for the hashed doc (pre-refresh git-log verify → hand-edit → doc+manifest one commit). **No new tooling** (Improvements reviewer: no findings) — reuses shared-rules link propagation, the template symlink pool, the `d-implementation-plan.md.template` HTML-comment pattern, and the existing hash-updates procedure. "None stated" as an explicit dual-capture fallback is load-bearing: without it the instruction would pressure the agent to invent deliverables for empty requests. TC-1…TC-10 (binding, deterministic grep/sha256/validate/render) all PASS incl. a fresh `template-copier` render showing the new dual-capture Goal placeholder; TC-11 (live replay of the trigger scenario) is confirmatory/soft, not gated — prose-instruction reliability is provable only in field use. `cwf-manage validate` OK; all 7 changeset reviews (5 at f, 2 at g) `no findings`. **Two advisories surfaced and deliberately not fixed** — expanding scope to fix them would re-commit the very boundary-stretch error the task exists to prevent: (1) robustness — `planning.md`'s means-fence points to "the requirements *and implementation* phases" but the challenge-requirements text lives only in `requirements.md`, a dangling cross-reference (not a broken path) for the three types without a `b` phase; (2) security category-e — the new verbatim `Explicit request` field is safe today (read as plan content, never drives a tool call) but any future consumer keying logic off it rather than the validated task number would create an injection vector. Both backlogged.

## Task 225: gate exec phases on terminal subtask status

### Status: Complete (2026-07-10)
### Duration: bugfix phases a,c,d,e,f,g,j in one session; ~1.5h against a 0.5-day estimate (~60% under).
### Impact: Reported by a CwF user against their own Task 68 — the parent squashed its branch at retrospective (`j`) while subtask 68.1 was still open, rewriting the commit 68.1 was branched from; 68.1's merge-base regressed to the grandparent task and the suggested `git merge --ff-only` could no longer fast-forward. **The reported symptom is downstream of the real defect**: no CWF phase ever consulted a task's children. `cwf-retrospective` Step 6 gates on "all phases Finished (100%)", but *phases* means this task's own `a`–`j` files; `workflow-manager status --workflow` never descends into children. A parent with a live subtask therefore read as 100% and walked through `f`, `g`, `h`, `i`, `j` unchallenged. Blast radius stated accurately: the damage was **loud, not silent** — the ff-only discipline contained the corruption; what the user lost was a documented path forward, and their agent had to invent `git rebase --onto` unaided. **The omission was inconsistent with CWF's own precedent** — `find_children()` already existed and `/cwf-delete-task` already used it as a hard refusal ("Check 5: leaf"); the exec phases simply never called it. Settled position (user, twice, decisively): **a subtask is blocking by definition** — work that does not block the parent's deliverable is a top-level follow-up or a backlog item, not a child — so the gate is strict, and the only non-blocking child states are the three terminal statuses **Finished / Skipped / Cancelled**. New `CWF::SubtaskGate` (`nonterminal_children`, `phase_is_gated`, `format_blocked`) built **entirely from existing parts**: child discovery reuses `find_children()`, and `TaskState::_is_closed()` — private and unused — was promoted to the exported `status_is_terminal()` with `_is_closed()` now delegating, so **one status list exists, not two**. Wired at **three chokepoints, each guarding a distinct failure at a distinct time** (a reviewer's suggestion to cut one as redundant was declined): the SKILL Pre-Step refuses phase *entry* across all five gated skills; `cwf-checkpoint-commit` refuses before `status_set` stamps a phase `Finished`; `checkpoints-branch-manager create` refuses immediately before the destructive `git reset --soft` squash — an agent ignoring a SKILL instruction still hits the second, a hand-run squash still hits the third. **The high-priority mid-`f` deadlock risk was designed out**: the gate fires on phase *entry*, never on subtask *creation*, so the documented decompose-during-`f` flow survives. Only **direct** children are checked — each child's own gate holds back its grandchildren, so no recursive descent and one `glob` per phase entry. Fail-closed throughout and tested as such: an unresolvable task `die`s rather than reading as "no children" (TC-14); an unrecognised phase is exit 1, never a silent ungated pass (TC-18). **Terminality is not completeness, and a percentage cannot express the difference** — the end-to-end probe made it concrete: `workflow-manager status 225 --workflow` reported throwaway subtask `225.1` at **100%** while every one of its six phases read `Cancelled (0%)` (`state_done` MINs over `status_percent`, and `_is_closed` folds all three terminal states to 100), so a `state_done == 100` gate would have been correct on that input **by accident** and wrong in general; the gate instead asks the right question per-file and gets the offending phase names for free. That probe reproduced the **real** user failure rather than a mock: all three chokepoints refused `225.1` (exit 3, exit 3, die), **no `-checkpoints` branch was created and HEAD did not move**; cancelling the child's phases opened the gate; the probe was deleted, `validate` clean, tree clean. **Two plan defects, both caught by execution rather than by review.** (1) The testing plan's flagship reproducer *does not reproduce*: TC-11 asserted a child holding only a Finished `a-task-plan.md` aggregates to `state_done == 100`; it aggregates to **0**, because removing `f-implementation-exec.md` also flips the version detection at `TaskState.pm:304` to v2.0, whose filenames differ entirely, so **no** expected file is found. The fix was unaffected but the *claim* narrowed to "any partial file set **that retains the format-marker file**" — TC-11 was rewritten (asserting `state_done == 100` inline *before* asserting the gate blocks, so a `state_done`-only gate provably cannot pass) and TC-11b added for the misdetection variant. (2) The implementation plan had a label fix **backwards** — it instructed changing `cwf-retrospective/SKILL.md:41` "Step 6"→"Step 7", but the SKILL was internally consistent and the `retrospective-extras.md` heading was the one off by one; followed mechanically it would have broken a correct file to match an incorrect one. **A reviewer was right about a defect and wrong about its exploit path**: best-practice flagged `/^[a-j]$/` accepting `"f\n"` (`$` matches before a trailing newline) which, feeding `phase_is_gated`'s exact hash lookup, is a **silent ungated pass** — the worst failure mode for a gate — but reported it as reachable via the gate CLI. It is not (`CWF::Options::parse` anchors its own arg regex and strips the newline first, probed not assumed); the reachable path was `cwf-checkpoint-commit`'s raw `@ARGV` read. Both validators re-anchored `\A…\z` with `/a` (so `\d` no longer admits Unicode digits) and non-capturing groups; TC-24/TC-25 pin both. Improvements and misalignment **independently converged** on the same duplication — the gate had copied `_get_all_statuses`' format-detection block with "these must not disagree" enforced only by a comment — fixed by extracting `CWF::TaskState::expected_files()`. Two findings declined with recorded rationale (`die` over `Carp::croak`; the CLI's `eval` collapsing exceptions to exit 2, already fail-closed). The halt message is read **verbatim into agent context** by the SKILL Pre-Step, making it a prompt-injection surface: the one free-text field (child status, returned unvalidated by `status_get`) is clamped — whitespace collapsed, truncated to 32 chars — with a code comment naming why; every other interpolated field is constrained by `parse_dirname`'s regexes, and the security reviewer traced each one. `CWF::TaskState` + `workflow-manager` + `cwf-checkpoint-commit` + `checkpoints-branch-manager` sha256 refreshed and the two new files recorded, all in the exec commit per the hash-updates convention (`SubtaskGate.pm` 0600 as a `use`d module with no `permissions` key; `workflow-manager.d/gate` 0500). `t/subtask-gate.t`: 19 top-level subtests, TC-1…TC-25. Full suite `Files=78, Tests=1073, PASS`; `cwf-manage validate` OK. All 7 changeset reviews (5 at f, 2 at g) clean bar the fixed/declined findings above. **Deliberately deferred, not overlooked**: `Finished` does not imply `merged` — a child can be Finished and unmerged, in which case `j` still squashes the parent and strands the child's base. The gate is a *status* invariant testable against the file tree; merged-ness is a *git* invariant needing branch resolution, ancestry checks (`TaskPath::parent_branch_ancestry()` looks right), and a policy for already-deleted child branches. Backlogged as a Medium bugfix rather than doubling the surface and delaying the fix for the common case.

## Task 224: resolve tool-check hook paths from git root

### Status: Complete (2026-07-10)
### Duration: bugfix phases a,c,d,e,f,g,j in one session; estimate <1 day, on target.
### Impact: Delivered backlog item **R7** (Task 219, High) — **with a corrected premise, which is the headline.** R7 read *"tool-check hook fails open from a subdirectory; resolve the hook path from the git root, not cwd"*, `[S, severe]`. Planning verified rather than assumed, and found the stated defect **already fixed**: Task 204 (`d985db3`, 2026-06-15) landed the `${CLAUDE_PROJECT_DIR}/` prefix on every hook `command` **one day after** R7's evidence was recorded (2026-06-14), and `find_git_root()` was never implicated (probed directly: returns the repo root from a nested subdirectory, `undef` only outside a repo). The entry had aged into a description of a live severe defect that was already closed. Rescoped with explicit user approval onto the two live residuals of the same class: **D1** — `.cwf/docs/workflow/stop-hooks-framework.md:164` still taught the bare-relative registration, so an operator hand-registering from the shipped example reproduced the original fail-open (that doc sat wrong for ~a year; Task 204 never touched it); **D2** — the prefix was enforced only inside one generator and its unit tests, so a hand-edited `.claude/settings.json` silently reverted to a disabled hook with **no signal** — the same "guard is off, no signal" property that made R7 severe. New `CWF::Validate::Hooks` (145 lines, mirrors the `CWF::Validate::Agents` precedent: `@EXPORT_OK qw(validate)`, same six-key violation hashref, same `cmd_validate` composition) wired in via **two lines** in `cwf-manage`. **The obvious invariant was the wrong one**: "the command must *start with* `${CLAUDE_PROJECT_DIR}/`" is false — the shipped rules-inject hook registers as `cat "${CLAUDE_PROJECT_DIR}/.cwf/rules-inject.txt" 2>/dev/null || true`, prefix **mid-string** — so the enforced invariant is *no bare `.cwf/` reference anywhere in the command*, a fixed-width negative lookbehind (no ReDoS). The scan is scoped to the `hooks` tree only: both settings files carry `permissions.allow` entries like `Bash(.cwf/scripts/hooks/…)`, which are **match patterns, not exec paths**, and a whole-document scan would have fired on ~6 legitimate entries on day one — a validator that false-positives gets switched off, not fixed. **Surface, never repair**: no `--fix`, not wired into `fix-security`. The validator **must never die** (unlike its `Agents.pm` sibling, which dies on `open`): a die would abort the eight validators composed alongside it, so every bad-file path (absent, unreadable, symlinked, non-regular, malformed) degrades to one `json-parse` violation — and the **absence gate is pinned inside `validate`**, not collapsed into the reader's `$ok`, because absent and unreadable both return `$ok = 0` and driving the violation off `$ok` alone would emit a spurious violation for every project lacking the gitignored `settings.local.json` (the common case). Plan review caught three defects that would have shipped: a **false claim in the design** that TC-3 bound the generator and validator literals together (it tests a hardcoded string shape — TC-9 exists because of that catch); a no-die argument that defended only the JSON *decode*, leaving a present-but-unreadable file to die on `open`; and TC-6b going red under root (`chmod 0000` does not stop root — added the `$> == 0` skip guard). Testing exec went beyond the plan: TC-8 and TC-9 are "the source still says X" assertions that pass just as happily against a typo'd pattern matching nothing, so both were **mutation-verified** (regress the real site → observe red → revert → observe green; the generator is `0500`, so TC-9's mutation needed a temporary `chmod u+w` and a re-clamp to the recorded value). A **live-tree system test** proved the wiring that fixtures structurally cannot: mutating one real hook command in `.claude/settings.json` yielded **exactly one** violation, confirming D2 end-to-end since the `permissions.allow` pattern was not flagged. Incidentally established that `.claude/settings.json` is a sandbox bind-mount — `git checkout --` fails (*Device or resource busy*), `cp` fails (*Read-only file system*), only the Edit tool writes it. Hashes for the new module (no `permissions` key — a `use`d module, not an executable) and `cwf-manage` refreshed in the same commit after a per-file `git log` check proved no intervening change; perms needed no clamping. 46 new assertions; full suite `Files=77, Tests=1054, PASS`; `cwf-manage validate` OK. All 7 changeset reviews (5 at f, 2 at g) clean bar one adopted best-practice finding (unchecked `close` on a write handle in a test fixture helper) and one accepted, twice-raised style divergence (regex omits `/x`, uses `\.` over `[.]` — a conscious match with the sibling `Agents.pm`). Backlogged: a doc-drift check for when a task changes an enforced shape (this task's root cause), and sandbox null-routing polluting `git status`.
...
### Status: Complete (2026-07-08)
### Duration: feature phases a–j in one session; Low–Medium complexity, on target.
### Impact: Delivered backlog item **R2** (Task 219, High): stopped non-terminal and non-canonical `Status` values leaking onto committed CWF phase files by making every phase resolve its **own** wf-step file to a terminal status at its checkpoint — while **retaining** the retrospective status sweep as an explicit defence-in-depth gate (the user constraint: inconsistent phase state causes false positives in the hierarchical context scripts, so the belt-and-braces sweep stays). The premise was partly stale — `cwf-checkpoint-commit` (Task 102) has force-stamped the current phase `Finished` since then, which is why this repo's recent tasks are all-terminal — but the leak is live off the script path and in older CWF, confirmed three ways: cross-project LMM retros (`Backlog`/`Planning`/`Implemented`), real committed omnilsp stamps using the phase *name* as a status (`Design`×3, `Requirements`, `Planning`), and the shipped `f-implementation-exec.md.template:20` hint `Update status to "Implemented"` (non-canonical). Six production files, +137/−29, **exactly one hashed-file edit**: (D1) deleted the `f:20` `"Implemented"` hint — the only non-canonical token in the pool — keeping `g:20`'s canonical `Testing`/`Finished` hint; (D2) reworked the retrospective checkpoint in `retrospective-extras.md` so `j` stamps its own `j-retrospective.md` `Finished` via an `&&`-chained `cwf-set-status` **hard precondition** (a non-zero stamp aborts the commit rather than being papered over by a green one — surface, never smooth; the Step-7 sweep runs before it and cannot backstop it, the hook is the remaining backstop); (D6, folded in at pre-exec review) strengthened `.cwf/scripts/hooks/stop-stale-status-detector` from `Backlog`-only to flag **any** non-canonical status via a new caller-guarded, unit-testable `is_flaggable` predicate that reuses the already-exported `CWF::TaskState::status_is_valid` — no `CWF::TaskState` change, no new helper, no new public API. Guarded by a new reuse-based `t/status-terminality.t` (template hygiene incl. a deterministic red-on-seed proof: reintroduce `"Implemented"` → RED naming the token → revert → GREEN → clean diff; plus the `is_flaggable` decision) and a `_is_closed` terminal-set subtest in `t/task-state.t`. The `Testing`-during-g false-positive risk was neutralised by constraining only the *committed completion* state to terminal, leaving transient `In Progress`/`Testing` untouched. Skipped-phase automation was considered and documented out (best part is no part — skipped phases have no file under the symlink model). Hook sha256 refreshed in the same commit (perms restored to recorded 0500); `cwf-manage validate` OK. All 7 changeset reviews (5 at f, 2 at g) `no findings`; full suite `Files=76, Tests=998, PASS`. This task's own j-retrospective checkpoint was the first genuine end-to-end exercise of the new `&&`-chained stamp (closing the TC-5 "mechanism-only" caveat). Backlogged: a low-priority look at why `workflow-manager status` reads 9/10-terminal phases as 25%.

## Task 221: seed exclude-path defaults, raise review cap to 1000

### Status: Complete (2026-07-08)
### Duration: feature phases a–j in one session; Low complexity, on target.
### Impact: Delivered backlog item **R1** (Task 219, High) plus a user-requested cap bump. Task 218 already shipped the *mechanism* — `security.review.max-lines-exclude-paths` (git `:(glob,exclude)` pathspecs, no Perl-side matching) and the `--max-lines` > config > built-in-default precedence — but the keys shipped **unset** (every project re-derived them) and the built-in default of 500 tripped semi-frequently, especially when a review diffs all the way back to a task's baseline. This task supplies (a) a seeded 20-glob default in `.cwf/templates/cwf-project.json.template` (18 generic test/generated/vendored globs + scoped `*.md`/`docs/**/*.md`) alongside a `_security-review-note`, and (b) raises the built-in cap 500→1000 in the one live constant `$DEFAULT_MAX_LINES` (`security-review-changeset:101`), syncing the three human-readable copies (banner, comment, two `security-review.md` prose mentions) and **rewording** the self-note to drop its hardcoded number so that copy can never drift again. **No new runtime code** — reuses Task 218's `max_lines_exclude_paths()` engine (Improvements reviewer: no findings). Reach is deliberately asymmetric: the cap bump reaches **every** updating install (a code-level fallback), while the seeded excludes reach **new inits only** (`cwf-init` never overwrites an existing `cwf-project.json` — no silent security-config rewrite). Design D2 kept `max-lines` **out** of the template (a helper default, not seeded) so installs inherit the built-in. Dog-fooded by dropping this repo's now-redundant explicit `max-lines: 1000`, keeping its `t/**`/`implementation-guide/**` excludes. **Fail-open tradeoffs surfaced, never smoothed**: the cap loosening (a 501–1000-line changeset now auto-reviews instead of tripping exit 2 — the *safe* direction, since the cap gates review *invocation* not content) is disclosed in the h-rollout with the mitigation (pin `security.review.max-lines: 500`); the markdown discount is disclosed in the template note (excluded paths are still fully reviewed, so a large adversarial-markdown changeset stays under the cap and is still auto-reviewed). One defect, caught in review and fixed in-phase: the best-practice reviewer found `TC-SEED-GUARDRAIL`'s git pipe `close` was unchecked, so a non-zero git exit would read as zero hits → a **false-PASS** on the FR3 security guardrail — fixed by dying on a non-zero close status (`$? >> 8`); testing-exec reviewers then confirmed the fix in-diff. The 32-way `500`-literal triage in `t/security-review-changeset.t` held exactly: TC-DEFAULTCAP re-baselined (520→1020, asserts >1000) + new TC-CAPBOUNDARY (1000 pass / 1001 exit 2); behaviourally-neutral config-cap prose → 1000; TC-CONFIGCAP4 (explicit-flag test), the TC-DOCS negative guard, and the `[500]` ref-type literal correctly **kept**; new TC-SEED-VALID/EXCLUDE/DOC/GUARDRAIL exercise the seeded globs through git's real engine (incl. `seeded_exclude_globs()` loading from the shipped template, not hardcoded — guards template/test drift). `security-review-changeset` sha256 refreshed in the same commit per the hash-updates convention (perms stay 0500); `cwf-manage validate` OK. Full suite `Files=75, Tests=979, PASS`; all 7 changeset reviews (5 at f, 2 at g) clean bar the one adopted best-practice finding above. BACKLOG R1 marked delivered (High-priority ordering advanced to R2/R7).

## Task 220: opt-in tool-check hook seed and toggle

### Status: Complete (2026-07-07)
### Duration: feature phases a–j in one session; Medium complexity, calendar untracked per Task 219 S7.
### Impact: Made the shipped-inert Task-201 Bash tool-check framework opt-in usable without hand-authoring a settings file. Three parts, one DRY seed/toggle path: (1) an `active` kill-switch the hook honours **fail-open** — a pure early-return before any rule load/eval — resolved over **trusted layers only** (project-local > user-global; the checked-in layer's `active` is **ignored** to close clone-suppression) and counting only a JSON boolean (`JSON::PP::is_bool`, guarding against a Perl-truthy `"false"` string), defaulting **true** so it only ever suppresses and composes with the zero-rules no-op; (2) a regex-only starter set (`no-sed-line-range`, `no-cat-pipe-grep`, `no-find-exec`) installed by a new `.cwf/scripts/command-helpers/tool-check-seed` helper (0500, hash-tracked) via symlink-safe atomic writes (`O_EXCL` + `rename`, 0600, per-level `-d && !-l`), add-missing-by-id so a re-seed never clobbers a user edit; (3) `/cwf-config tool-check on|off|seed` + a `/cwf-init` opt-in (default **decline**, non-fatal) sharing that one helper. The a-plan's `enabled` enable-gate was deliberately refined in requirements/design to the `active` **kill-switch** (default-true) — an enable-gate defaulting false would have broken the "seed then it just works" flow; every original AC still held (traced AC1–AC9). Two exec-review findings adopted: robustness caught a **real bug** — `effective_active` wrapped `read_settings` in `eval`, but the helper failed via `die_err` → `exit 1`, which `eval` cannot trap, so a symlinked/corrupt user-global file turned an already-completed write into a misleading `exit 1`; fixed with a `soft => 1` read mode + regression TC-S9. Improvements caught a duplicated `trusted_layers` (hook-side + helper-side) → hoisted into the pure `CWF::ToolCheck` module (exported, unit-tested via TC-U3b) so the trust boundary can no longer drift. Misalignment was accepted-with-rationale (not adopted): reusing `CWF::ArtefactHelpers::atomic_write_text` would lose the NFR4 symlink defence (it uses `make_path`, does not reject an `-l` target) — coupling a security-sensitive writer to a general-purpose one is the wrong trade; a converge-the-three-writers candidate is backlogged instead. **Rollout decision (h)**: this repo ships the *mechanism only* — no checked-in starter set committed for CWF's own development (the maintainer already runs a richer user-global set); adoption stays a backlog item. `CWF::ToolCheck.pm` + `pretooluse-bash-tool-check` sha256 refreshed and the new helper recorded, all in the exec commit per the hash-updates convention; `cwf-manage validate` OK. 39 new tool-check subtests (unit 13 + hook 17 + helper 9); full suite `Files=75, Tests=970, PASS`, no Task-201 regression. All 7 changeset reviews (5 at f, 2 at g) clean bar the two adopted/one accepted findings above. Dogfooding: the maintainer's live user-global rule denied a stray `head` during exec. BACKLOG R3 premise corrected (user-global ruleset predates the "re-derive" claim) and the standalone seed item updated to reflect the settled h-phase decision.

## Task 219: aggregate cross-project retrospective lessons

### Status: Complete (2026-07-07)
### Duration: discovery phases a–g,j in one session; estimate 1–2 days, within estimate (High complexity).
### Impact: Discovery — assessment-only (no CwF code/doc changes). Mined the lessons-learnt signal across every CwF-using project in `~/repo/*` (11 repos) plus Claude Code session logs and LMM, to find generalizable improvements along three axes: token efficiency, permission-prompt reduction, and SDLC friction. Deliverable is a §1–§4 assessment in `f-implementation-exec.md`: coverage reconciliation, per-axis findings, 14 ranked (impact-desc/effort-asc) recommendations each with a safety↔momentum tradeoff, and 14 seeded follow-up tasks. Method: a map-reduce over the retrospective corpus — a deterministic survey established the denominator (601 tracked `{h,j}-retrospective.md` files, both conventions), 15 sharded **read-only** extraction agents + a session-log miner + three per-axis LMM sweeps returned bounded JSON digests, and a two-stage (per-axis then global) reduce clustered → corroboration-filtered (≥2 external projects ⇒ "general") → novelty-diffed against the codified baseline (`MEMORY.md`, feedback memories, `error-patterns.md`, both convention dirs) → ranked. Injection contained by construction (read-only agents, sole-writer orchestrator, dispatch-key digest filenames). The **reconciliation step earned its place**: it caught two shard mis-slices and a range gap (~83 retros) that 3 gap-fill extractors then closed, lifting coverage to 541/601 (~90%); the residual ~60 are subtask retros nested in covered ranges, logged not smoothed. Top cross-project findings (5–8 project corroboration): the security-review 500-line cap trips on test/generated/doc lines (~40 task instances across 7 projects — partly addressed by Task 218's configurable cap but not defaulted at init); shell-operator/command-decoration permission prompts (~10 scattered avoidance rules, the Task-206 injection hook shipped only here); non-terminal Status leaking onto committed phase files (8 projects); a CwF-upgrade-friction cluster (`.cwf/version` read-tree churn, lock-before-clean-check, sandbox blocks in-session update, 5 projects); late architecture/surface decisions causing design churn (6 projects). Two severe single-project signals: a `TRUNCATE` test wiped ~500 MB of production embeddings via a `TEST_DATABASE_URL`→prod fallback (lmm), and a **tool-check hook fails open from a subdirectory** (session-log evidence). Plan review earned its keep — the misalignment reviewer caught the `h-retrospective.md` second-convention undercount at plan time, correcting the denominator from ~558 to 601. Validation TC-1…TC-9 all pass (TC-8 with a documented scratch-persistence deviation — direct fan-out held digests in context, persisted one to scratch); all 5 exec + 2 testing changeset reviewers `no findings`; `cwf-manage validate` OK. The 14 recommendations are added to the backlog as a Task-219 follow-up group; an incidental `Wide character in print` UTF-8 defect in `plan-mechanical-check`/`ArtefactHelpers.pm` was observed and seeded as a bugfix.

## Task 218: configurable changeset-review max-lines cap

### Status: Complete (2026-07-05)
### Duration: feature phases a–j in one session; estimate <1 day, Low complexity, on target.
### Impact: Let a project set the changeset-review production-line cap in `cwf-project.json` (`security.review.max-lines`) instead of relying only on the helper's hardcoded 500, so the value survives CWF upgrades. `security-review-changeset` now resolves its effective cap as `--max-lines` (CLI) > `security.review.max-lines` (config) > built-in default, the default single-sourced in one `$DEFAULT_MAX_LINES = 500` constant consumed by both live-code sites and interpolated into the POD (the plain-`#` header banner repeats 500 literally — a documented manual dual-write). New `config_max_lines()` mirrors the sibling `max_lines_exclude_paths()`: eval-guarded `CWF::Versioning::read_config`, `ref … eq 'HASH'` navigation down `security.review.max-lines`. Fail-safe by construction — a missing key or JSON null degrades **silently** to 500; a malformed value (non-integer, boolean, array, zero, negative, leading-zero) **warns naming the key only** (no value echo, NFR4 info-leak) and degrades to 500; ambiguity never fails open-large. The CLI/config asymmetry is deliberate and pinned: an invalid `--max-lines` CLI value stays **fatal** (exit 1) while a malformed config value degrades. This repo's own `cwf-project.json` cap raised to 1000. `security-review-changeset` sha256 refreshed in the same commit per the hash-updates convention (perms stay 0500); pre-refresh `git log 9972522..HEAD` confirmed no un-blessed intervening edits and the live digest matched before the refresh. Design D2 declined to extract a shared `review_config()` wrapper at the two call sites (Rule of Three not met; would hide per-key validation). New TC-CONFIGCAP1–10 cover the full precedence matrix (config-no-CLI, 600-line pass at cap 1000, CLI beats config, explicit `--max-lines=500` beats config) and every invalid-config equivalence class plus the CLI-fatal-despite-valid-config regression; renamed from the e-plan's TC-CAP7–16 to avoid colliding with existing TC-CAP7/8/9, and the silent-default fixture reduced below 500 so the exit-2 boundary does not mask the signal. 947 tests across `t/` pass; all 7 changeset reviews (5 at f, 2 at g) clean; `cwf-manage validate` OK. One incidental regression surfaced and was fixed in-phase: a fix-on-sight `cwf-manage fix-security` on two Task-217 files clamped 0600 → 0400, under the recorded 0444 floor, breaking `t/cwf-manage-fix-security.t` TC-8 — `fix-security` only strips excess bits and never raises, so a recorded-**floor** file needs `chmod <recorded>` directly (now recorded in memory). Deliberately in-repo-config-only: if a future task reads the key from an out-of-tree source (env override, user-global config), the self-referential trust boundary (b-requirements NFR4 / c-design D3) must be re-audited first.
...
### Duration: bugfix phases a,c,d,e,f,g,j in one session; estimate ~1 day, ~0.5 day actual.
### Impact: Fixed CWF's per-task scratch directory resolving to a read-only path under Claude Code's bash sandbox. Root cause is a sandbox asymmetry: Claude Code **hooks run unsandboxed** (`$TMPDIR` unset → `/tmp`) while the **Bash tool runs sandboxed** (`$TMPDIR=/tmp/claude-<uid>`, `/tmp` read-only). The `userpromptsubmit-context-inject` hook resolved scratch in the host env and froze a `/tmp/cwf-…` literal that the in-sandbox shell could not `mkdir`. The a-plan anticipated a persisted `.cwf/config.local.json` `$TMPDIR` cache (Approach B); the design phase pivoted to **Approach A** — a self-validating probe in `CWF::Common::scratch_parent` — which removed the cache, the hook edit, and the `.cwf/.gitignore` artefact, eliminating three plan risks (cold-start lag, accidental cache commit, concurrent writers) outright rather than mitigating them. The resolver now branches three ways: `$TMPDIR` if set (in-sandbox hot path, disk-free) → else probe `/tmp/claude-$>` (numeric effective uid, no external string) adopted only if `!-l && -d _ && -w _` (single `lstat`, symlink-reject parity with `scratch_dir`'s leaf guard) → else today's `/tmp` fallback. The probe target is the overridable package var `$SANDBOX_TMP_PROBE` so every arm is testable hermetically. No hook change and no `($parent,$err)` contract change — the resolver fix flows through to the hook's emitted line. Collateral: `t/backlog-bootstrap-changelog.t` hardcoded `tempdir(DIR => '/tmp')` (same bug class, surfaced by the full-suite gate) was fixed on sight to honour `${TMPDIR:-/tmp}`; grep confirmed it was the only such site. `tmp-paths.md` corrected (`/tmp/claude` → `/tmp/claude-<uid>`; the stale "no sandbox-detection branch" rule rewritten to describe the probe). `Common.pm` sha256 refreshed in the same commit per the hash-updates convention (lib `.pm`, perms stay 600). Verified live end-to-end: with `$TMPDIR` deleted the resolver returned the in-sandbox writable base, not the stale read-only literal. New TC-9..TC-14 cover env-wins / probe-adopted / absent / non-writable / symlink / empty arms (TC-12/TC-13 self-verify EUID and symlink support rather than silently skip); 14 in `t/scratch.t`, 937 across `t/`; `cwf-manage validate` OK. All seven changeset reviews clean bar one accepted advisory — the `${TMPDIR:-/tmp}` idiom is now open-coded in four sites (crosses Rule of Three); an exported `CWF::Common::tmp_base()` helper is backlogged.

## Task 214: classify auto-discover review outputs

### Status: Complete (2026-06-27)
### Duration: chore phases a,d,e,f,g,j in one session; estimate ~0.5 day, on target.
### Impact: Removed the blocking permission prompt that fired on every exec-phase Step-8 review. The exec skills previously classified each reviewer's output with a per-file shell loop (`for … ; do security-review-classify < "$f"; done`), whose `$var` redirect does not match the `Bash(.cwf/scripts/command-helpers/security-review-classify:*)` allowlist entry → a prompt every run. `security-review-classify` gains an additive `--dir <DIR> --phase <PHASE>` discovery mode: it locates every `*-review-output-<phase>.out` regular file in the (trusted, per-task scratch) directory, classifies each, and prints one `<reviewer>: <token>` line in lexical order — a single literal argv that matches the allowlist and raises no prompt. The existing `stdin → one token` contract is byte-identical (the SubagentStop guard hook and single-file callers are untouched), achieved by extracting the block-walk verbatim into a shared `classify_text()` so the Task-162 single-parser invariant holds — discovery mode adds I/O around the one parser, not a second one. Discovery does a one-pass `[reviewer, file]` capture on `^(.+)-review-output-\Q$phase\E\.out$` (the `-review-output-` infix excludes the co-located larger `-review-changeset-` diff inputs; `\Q$phase\E` scopes and quotes the phase), filters to `-f && ! -l` (skips a matching symlink/subdir), and surfaces a stderr `[CWF] WARNING:` on zero matches or a per-file read failure (that file's line is `error`, never a silent drop); exit 0 always. Both exec skills (`cwf-implementation-exec` 5-reviewer, `cwf-testing-exec` 2-reviewer) now call the single invocation, with an explicit `<reviewer> → ## Heading` map and a launched-vs-classified cross-check (a launched reviewer with no line is recorded `error`); the two `.cwf/docs/skills/*-review.md` snippets document both modes. `script-hashes.json` refreshed in the same commit (perms stay 0500). New TC-D1..TC-D6 (happy-path/lexical-order, phase scoping, zero-match, symlink/non-regular skip, per-file open failure, arg errors incl. the previously-uncovered unknown-flag rejection) join the byte-identical stdin regression cases — 30 in `t/security-review-classify.t`, 931 across `t/`; `cwf-manage validate` OK. The feature was dogfooded in this task's own f- and g-phase reviews (5 then 2 reviewers classified in one prompt-free invocation). All seven changeset reviews clean bar one accepted advisory: the `--dir`/`--phase` interface diverges from the sibling `security-review-changeset` (`--wf-step` + internal `scratch_dir()`), backlogged as an optional alignment follow-up.
...
### Duration: a–j across ~4 calendar days (2026-06-18 → 2026-06-22), a few sessions; estimate 1–2 days, active effort under estimate. No scope change.
### Impact: The reuse/reliability/alignment review lenses that already run on plans now also run on the implementation diff. A CWF user reported these three reviews were not firing after implementation-exec — correct: only `cwf-security-reviewer-changeset` and `cwf-best-practice-reviewer-changeset` existed as changeset reviewers; the three lenses were plan-reviewers only. Adds three new agents — `cwf-improvements-reviewer-changeset` (reuse), `cwf-robustness-reviewer-changeset` (reliability), `cwf-misalignment-reviewer-changeset` (alignment) — each cloned from the Bash-free `cwf-best-practice-reviewer-changeset` (the `{bp_context_file}` input and its read-step dropped, the lens ported from the matching plan reviewer, the `cwf-review` verdict block kept byte-identical so a missing/malformed block classifies as `error`). Tool grant is `Read, Grep, Glob, LSP` — Bash withheld, a strictly narrower surface than the security reviewer. Step 8 of `cwf-implementation-exec/SKILL.md` is rewritten from two reviewers to five: the single `security-review-changeset` run now drives the security and three lens sections across every exit state (on-`main` records five sections, empty changeset records five, helper error records four as `error`); the MAP grows to ≤5 parallel calls; each of the five sections is classified and recorded independently so one reviewer's `error` never suppresses another (FR7). `cwf-testing-exec` is untouched — the three lenses never run after testing-exec (FR4, asserted as a positive invariant). The three agents are hash-registered at `0444` in `script-hashes.json` in the same commit; the SubagentStop verdict guard is untouched, so the advisory lens verdicts never block (FR5, verified live). New test `t/exec-changeset-reviewers.t` (11 subtests) covers the file invariants, the 2→5 degradation paths, and classifier error-isolation; full `prove t/` 882 green (was 871); `cwf-manage validate` OK. All four exec changeset reviews (f and g, security + best-practice) returned no findings; the live five-reviewer MAP (run in h once the agents were session-loaded) returned no findings on four lenses and one advisory `findings` — the byte-identical verdict-block duplication — which correctly did not block.

### New Backlog Items
- Hoist the shared `cwf-review` verdict block and the "Bash intentionally withheld" paragraph into `cwf-agent-shared-rules.md` and de-dup the (now five) changeset reviewers (chore, Low) — agent `.md` files have no include mechanism, so the shared body is currently copied; hoisting would force re-hashing the two pre-existing changeset agents and editing the hash-tracked shared-rules doc, so it was deferred to keep Task 210 scoped. Re-surfaced live by the improvements reviewer in h.
...
### Duration: ~1 day (estimate 2-3 days, Medium; under estimate via wholesale pattern reuse).
### Impact: CWF gains a tag-aware best-practice reviewer across both the planning plan-review and the exec changeset review. Users supply best-practice tuples `{documentation: file/dir/URL, tags}` in a JSON config under project `.cwf/best-practices.json` and/or user `~/.cwf/best-practices.json`; the new Perl helper `.cwf/scripts/command-helpers/best-practice-resolve` (0500, hash-tracked) merges the two configs (project-wins on `documentation` collision; active-tags / url-allow-hosts unioned), computes the task tag set (project `active-tags` ∪ the task's `**Tags**` line), casefold exact-token matches entries, and resolves matched docs into a sentinel-wrapped manifest written to the per-task scratch dir. Two new agent definitions (0444, hash-tracked, tools `Read, Grep, Glob, LSP, WebFetch` — **no Bash**) consume the manifest: `cwf-plan-reviewer-best-practice` (prose findings into the plan-review reduce) and `cwf-best-practice-reviewer-changeset` (emits a `cwf-review` verdict classified by the existing `security-review-classify`). The feature is **fail-open and no-op when unconfigured**: a missing/malformed config or zero tag matches degrades to a clear "no applicable best practices", never a workflow block. Source resolution is security-gated: project paths are realpath-confined per-member (TOCTOU-safe, re-checked each recursion step); URLs are default-deny, fetched agent-side only when `allow-url-fetch` is set, https-only, and the host is allowlisted. A single normative doc, `.cwf/docs/skills/best-practice-review.md`, carries the helper contract, manifest discipline, prompt templates, config schema/precedence, and limitations; the planning and exec skills reference it (progressive disclosure). Three new `script-hashes.json` entries were authored in the same commit as the files.

### Notable
- **All review sub-agents run in parallel — the load-bearing structural decision.** The design (KD8) specified exec best-practice as a *serial* second review step; per explicit, repeated user direction it was restructured so the security and best-practice changeset reviewers launch **together in one message** (parallel), each classified independently — the SubagentStop verdict guard is name-matched to the security reviewer only, so no cross-contamination. The sole permitted serialisation is the two fast deterministic helpers in Prep, whose `.out` files are strictly-required agent inputs. Captured durably as memory `feedback-reviewers-parallel`; a design-checklist follow-up was filed.
- **Surface, don't smooth — held under the cap.** The exec security changeset tripped the production-weighted cap (`cap exceeded: 1082 production lines > 500`); rather than record a bare `error` and skip review of a security-sensitive helper, the reviewer was run deliberately on the full changeset → `no findings`. For testing-exec the production code was byte-identical, so the verdict was carried forward and the cap surfaced, not re-reviewed. Reinforces the existing cap-friction backlog lineage.
- **One correctness bug, caught in testing**: `realpath`/`Cwd::realpath` resolves a non-existent leaf under an existing parent without error, so missing-source detection needs an explicit `-e` check, not a truthiness check on the resolved path.
- **Untrusted-content threat model implemented as designed**: per-run random sentinel wraps verbatim source content and declares it UNTRUSTED; agents withhold Bash and never re-emit a fence; the classifier treats >1 `cwf-review` block as `error` so an embedded fence cannot forge a verdict. Residuals (DNS-rebinding past the host allowlist; non-crypto sentinel) documented as accepted, not silently carried.
- **Tests**: new `t/best-practice-resolve.t` — 21 subtests (TC-1…TC-22 less the planning-only TC-9, folded into TC-1), covering every config-failure branch, all three source kinds, path-escape/URL-gate/fence-forgery, and byte/member-cap truncation. Full `prove t/` green at **881 tests / 70 files** (one prior assertion in `t/security-review-changeset.t` was scoped from a broad `--phase` ban to the `security-review-changeset` invocation so the legitimate new `best-practice-resolve --phase=…` no longer false-positives — intent preserved). `cwf-manage validate` clean throughout (three new hash entries; no perm drift).
- **Carried to BACKLOG**: a design-phase checklist item for new-reviewer concurrency; a live-agent run of the reviewer against a populated `best-practices.json` (this repo exercises only the 0-match branch end-to-end).

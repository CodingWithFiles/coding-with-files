...
# Default status value mappings (if not in cig-project.json)
...
=head1 SYNOPSIS
    ...
    use TaskState qw(state_done state_achievable status_percent status_extract);
...
Loads mappings from cig-project.json workflow.status-values, falls back
...
sub _get_all_statuses {
    ...
    my $workflow_files;
    if ($is_v21) {
        $workflow_files = CIG::WorkflowFiles::V21::get_workflow_files($task_type);
    } else {
        $workflow_files = CIG::WorkflowFiles::V20::get_workflow_files($task_type);
    }
...
L<CIG::WorkflowFiles>, L<CIG::MarkdownParser>, L<TaskContextInference>

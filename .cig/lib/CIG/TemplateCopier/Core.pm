package CIG::TemplateCopier::Core;

=head1 NAME

CIG::TemplateCopier::Core - Version-agnostic template copying logic

=head1 SYNOPSIS

    use CIG::TemplateCopier::Core;

    my @templates = CIG::TemplateCopier::Core::discover_templates($templates_dir, $task_type);
    my %vars = CIG::TemplateCopier::Core::compute_variables(\%params);
    my ($created, $overwritten) = CIG::TemplateCopier::Core::copy_templates(\@templates, $dest, \%vars);
...
use CIG::TaskPath qw(get_parent);

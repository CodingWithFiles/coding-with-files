...
#
# Examples:
#   template-copier.pl --task-type=feature --destination=/tmp/test --task-num=17 --description="test task"
#   template-copier.pl --task-type=bugfix --destination=/tmp/test --task-num=1.2 --description="test" --format=json
#
# Exit codes:
...
sub find_templates_directory {
    # Try git to find repository root
    my $git_root = `git rev-parse --show-toplevel 2>/dev/null`;
    chomp $git_root;

    my $templates_dir;

    if ($git_root && -d "$git_root/.cig/templates") {
        $templates_dir = "$git_root/.cig/templates";
    } elsif (-d ".cig/templates") {
        $templates_dir = ".cig/templates";
    } elsif (-d "../.cig/templates") {
        $templates_dir = "../.cig/templates";
    } elsif (-d "../../.cig/templates") {
        $templates_dir = "../../.cig/templates";
        ...
        print STDERR "Searched: .cig/templates, ../.cig/templates, ../../.cig/templates\n";
        exit 2;
    }
...
sub validate_task_type {
    ...
    my @supported_types = @{$config->{'supported-task-types'}};
...
my ($created, $overwritten) = copy_templates(\@templates, $params{destination}, \%variables);

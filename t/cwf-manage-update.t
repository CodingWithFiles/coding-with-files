...
# --- TC-8 (Task 161): converged cwf-manage carries no copy-laydown dead code ---
subtest 'TC-8: removed copy-laydown subs and orphaned imports are gone' => sub {
    my $src = read_file("$REPO/.cwf/scripts/cwf-manage") // '';
    ok(length $src, 'cwf-manage readable');
    for my $sub (qw(update_copy copy_tree _escapes_src _collapse_dotdot
                    create_skill_symlinks create_agent_symlinks)) {
        unlike($src, qr/\Q$sub\E/, "no remaining reference to removed sub: $sub");
    }
    unlike($src, qr/^use File::Find\b/m,     'File::Find import removed');
    unlike($src, qr/^use File::Copy\b/m,     'File::Copy import removed');
    unlike($src, qr/^use File::Path\b/m,     'File::Path import removed');
    unlike($src, qr/^use File::Spec\b/m,     'File::Spec import removed');
    unlike($src, qr/^use File::Basename\b/m, 'File::Basename import removed');
};
